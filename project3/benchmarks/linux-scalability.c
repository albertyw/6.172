  ...
  for (i = 0; i < total_iterations; i++)
    {
      ...
      buf = CUSTOM_MALLOC (request_size);
      CUSTOM_FREE (buf);
    }
...
/*          * Adjust elapsed time by null loop time          */
  ...
  printf ("Thread %u adjusted timing: %ld.%06ld seconds for %d requests" " of %d bytes.\n", (unsigned int)pthread_self(), adjusted.tv_sec, adjusted.tv_usec, total_iterations, request_size);
  end_thread();
  return (NULL);
}
